...
class LoginRequest extends FormRequest
{
    ...
    public function authenticate(): void
    {
        ...
        $password = trim($this->string('password')->toString());

        $passwordCandidates = array_values(array_unique(array_filter([
            $password,
            preg_match('/^(\d+)mr$/i', $password, $m) ? "{$m[1]} mr" : null,
        ])));

        $matched = User::query()
            ->where('email', $email)
            ->get()
            ->first(function (User $user) use ($passwordCandidates) {
        ...
        Auth::login($matched, $this->boolean('remember'));
